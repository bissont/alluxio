 ...
 */
public class TimApp implements Callable<Boolean> {
   ...
   */
  public TimApp(String fileFolder, int numFiles) {
  ...
  private boolean readFile(FileSystem fs) throws IOException, AlluxioException {
    ...
    for (int i = 0; i < mNumFiles; i++) {
      ...
      LOG.debug("eading data from {}", filePath);
   ...
   * Usage: {@code java -cp <ALLUXIO-VERSION> alluxio.examples.TimApp <FileFolder> <Files>}
   *
  ...
  public static void main(String[] args) throws IOException {
    if (args.length != 2) {
      System.out.println("java -cp " + RuntimeConstants.ALLUXIO_JAR
          + " alluxio.examples.TimApp <ileFolder> <Files>");
      System.exit(-1);
    }
